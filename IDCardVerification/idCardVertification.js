window.onload = function() {
    ...
    input.onchange = function(event) {
        let res = idCardVertification(event.target.value);
        console.log(res);
    };
    ...
    function idCardVertification(id) {
        ...
        let newBirthday =
            birthDate.getFullYear() + '/' + Number(birthDate.getMonth() + 1) + '/' + Number(birthDate.getDate());
        ...
        */
        ...
        // 每一位对应的权重
        let idNumberWeight = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2];
        let remainder = ['1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2'];
        let sum = 0;
        // 计算总和
        for (let i = 0; i < 17; i++) {
            sum += +id.substr(i, 1) * idNumberWeight[i];
        }
        // 余数对应的最后一位校验码
        let check = remainder[sum % 11];
